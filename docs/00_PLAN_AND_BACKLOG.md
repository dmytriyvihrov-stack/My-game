...
|---|---|---|---|
| **238** | 🛒 **The wagon comes on the road** - the user, 2026-08-22: *"Хочу фургон, в котором до 5 апрейгдов можно сделать... апргрейд вместительности можно сделать несколько раз. Можно это назвать корпус/основа. Первый - чтобы игрок смог сделать в первой половине. Второй, ориентировочно перед последним боем. И третий - не успел. с каждым новым уровнем увеличивается стоимость"*, *"кнопку добавить в меню на глобал меп - справа от ресурсов... (может тоже в хедер) в меню инвенторя"*, *"это кенди - т.к. игрок видит последствия решений быстро и видит прогрессию силы отряда в общем"*, and the animal: *"опция животоного, которая тянет... сменить/улучшить можно токо через ивенты"*. 📐 **PROPOSED 2026-08-22, picture taken in the running build (`shots/238_wagon_mock_*`), NOT built.** His order, and the second entry in this bucket that adds a system by his ruling (#174 was the first) - ⚑ **though it is a MERGE as much as an addition**: salvage has NO in-run use today (Dima's run sat on ▤0 and read it as a promise about later), and the village wheelwright row and the between-runs BOLTED IN station both sell the same wagon and both fold into this one card. A chip right of the chest (measured x 1156..1194, in the bar's own 8px gap before MENU's 86px reservation) and a THE WAGON button in the sheet header; a `#wDlg` column card on the village shape with **no painting slot** (with one it is 658 into 684 and the prose scrolls 85px; without, 569 with every row visible). **THE FRAME 6 · 12 · 24 ▤, +2 seats a tier (13 → 15 → 17 → 19)**, priced against the curve measured on the map: a salvage-taking player holds ~12 by the Fen and 30-34 at the last fire (greedy 39 / 57-77), so I lands d7-10, II at the Door-Shrine if at most one fitting was bought first, III never on a normal run. The four fittings keep their shipped prices and are already wired in-run, so **no fight changes**; `FIT_SLOTS` is retired because the money is the cap (69 ▤ + 4 ◈ on the card against 30-34). The mule is a PLATE, not a greyed door, and swaps only through road doors (follow-up content). The forge stays between runs | ⛔ **three rulings first**: per-run (recommended) or permanent · +2 or +1 seats a tier · the mule's ladder (an ox: +3 ▤ on every field; a horse: one road a run a day shorter). Then build on a desk, about one session | [spec](archive/BACKLOG_ENTRY_SPECS.md) §238 |
