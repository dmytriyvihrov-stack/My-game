...
- **A caster carries a ladder, not one spell**, and the choice between them *is* the magic system:
  - **Ember** — 5 nerve, cheap, mostly ignores armour, and never counts against the per-turn cap. Always available.
  - **Unpick** — 9 nerve, no damage at all: strips 25 armour and 14 morale off one enemy. A setup spell that makes somebody else's turn better.
  - **Iron-Oath** — 12 nerve, and range **2**, so giving it means standing in the line.
  - **Blight-Wind** — 14 nerve, range **6**, crowd damage through armour.
  - **Wither** — 16 nerve, ignores armour entirely. The one you save for.
- **Captains on both sides.** Enemy forces have a captain with 3 actions to their rabble's 2, marked with a gold crest. Killing a captain deals a *much* larger morale shock to its whole side than an ordinary death — so "cut the head off" is a real strategy, and it is symmetrical: your Captain is the same target for them.
- **A boss does not out-damage you — it takes your line apart.** The Fen-Mother has **morale 260** (nothing routs her; a boss that can be worn down by nerve is not a boss) and **THE SOUND SHE MAKES**: no damage at all, 2 actions, and everyone of yours within 4 hexes tests their own nerve. Whoever fails drops below the rout line and runs for the edge. The check is against the morale you have *left*, so the second scream is far worse than the first — the fight itself has already softened everyone. The Captain gets a bonus to hold; anyone who cannot rout at all (*Stand Fast*, *Does not run*) takes the shock and stays, and the log calls that moment out, because it is exactly what those two things are for.
...
### Provisions are what the wounded eat

Wages buy the company bread; provisions are everything better than bread, and the sink that makes
...
> **Neither counted resource can go below zero** *(build 8f.51)*. Crowns and provisions have exactly
...
**Prices stay.** Crowns, provisions, gems and days are things you knowingly agree to — a merchant
...
| Nerve | one rung (🙂 *Happy*) | all five rungs, their % thresholds and what each costs |
...
Her nerve still grinds down — it drives her states and everything that reads them — but it can no
...
costs full nerve, the same death across the field about a third. A tight line is now a morale
...
focused by seven attackers lost its entire nerve in the round the lines met and routed at 72%
...
**And provisions still reach the battlefield.** Wages buy bread and nothing else, so a company that
has eaten only bread fights like one: **2 or fewer provisions and the whole line starts a fight 6
...
| **The Weight** | 3+ ogres within 2 hexes of you | **−9 morale every turn** — the nerve simply goes out of you |
...
**Morale being a damage stat is the interesting one.** It means the archer and the mage want it as much as the ogre does, and it ties the nerve system directly to output rather than leaving it as a pure survival stat.
...
- **~~Winds of Magic~~ — CUT.** *(User's call: "delete the Winds of Magic system entirely. I don't like it here. Wizards have their own problems.")*
  ...
  **What replaced it: casting costs the caster their own nerve.** Every spell spends the caster's morale — Ember 5, Unpick 9, Iron-Oath 12, Blight-Wind 14, Wither 16 — and **the second working of a turn costs 1.6× the first**. Combined with the per-turn casting cap, that is limit enough, and it is *personal*: a caster at the back of a winning fight can work all day; one in a losing fight is spending the exact resource they need to stay standing. A wizard's problems are their own.

  *Wind-touched* (−4 nerve per spell) and *Cold Hands* (−5) survive as the ways a particular person is better at bearing it.
...
- **Promotions, not XP bars.** After each battle **one random surviving character is promoted**, and promotions **alternate — a perk first, then a stat, then a perk again.** The order used to start on the stat, which meant a level-2 character sat looking at an open tier-2 tree with no point to spend on it and could not actually *choose* anything until level 3. The first promotion is the one that should feel like a decision.
- **Eight perks per class, in four tiers of two** (levels 2/3/4/5). The original four-per-class tree ran dry after about two promotions, and — worse — a perk promotion with an empty tree still *granted* a point and then silently skipped its own screen, so the point could never be spent and the promotion was simply eaten. **If there is nothing left to learn, the promotion becomes a stat point instead.** A reward must never evaporate.
...
- **Casters roll two of the four spell schools** (crowd / single-target / buff / debuff) when they're created, plus the Ember cantrip everyone has — so no two mages play alike, and a recruited mage is a genuine roll of the dice.
  - Randomising *who* gets promoted is the point: you do not build the party you planned, you build the party you were given. The roster becomes a history rather than an optimisation — the same job permadeath used to do, now carried by scars instead (see "Nobody on the roster dies in a fight").
...
| **The Cook-Fire** | 8 salvage | **+3 morale** every time the company stops, and a proper meal costs **one less provision** to lay out |
...
turn works · how a hit works · how nerve works · **nobody dies in a fight** · what the road costs.
