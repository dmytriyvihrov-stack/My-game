...
- **Death is the end of a chain, never a roll** — ***changed 2026-08-01 by the user's ruling on
  ...
  **SCARRED → MAIMED → the next one is the last one**. Going down is still a **scar** — permanent,
  ...
  > ⚠ **AND #34 IS NOT BUILT, SO AS OF 8f.114 NOTHING IN THIS BUILD KILLS ANYBODY ON YOUR ROSTER.**
  > Worth saying out loud because it surprised a session: `carryEverybodyOut()` says *"nobody is
  > lost"*, `afterBattle()` never splices a body, and a lost fight at THE CIRCLE goes through
  > `toBeaten`, which scars. **Scars are the death system today.** There are exactly three ways a
  > person leaves for good, and **`lostSoul()`** (#85, THE DREAM) is the one place that knows them:
  > you cut a returning veteran down at a fire, the sinkhole took them, or you dismissed them.
  > **The first of those is the only real crew death in the game, and the player commits it
  > deliberately.** When #34 ships, that function takes a fourth state and it outranks the rest.
