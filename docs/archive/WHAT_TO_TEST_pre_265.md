# What to test - the sections below #265

> Moved out of [`../WHAT_TO_TEST.md`](../WHAT_TO_TEST.md) on 2026-09-01, so the live bench is
> the week being played. **`tools/record.py` reads this file as part of the bench**, which is why
> it is a move and not the deletion the three earlier cuts made: write 4 of 4 is counted by
> asking whether an entry is named, and 80 deleted sections read as 70 missing writes.

> Everything here was written to be read with the game running, against a build that has moved
> since. Read it for what a feature was SUPPOSED to do, never as a description of today.
