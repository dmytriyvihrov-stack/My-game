...
| a `GEAR` row, an item icon, a price, a card that hands gear over | open the lab's **ARTIFACTS** tab (`tools/serve.ps1`, then `http://localhost:8777/tools/lab.html`) and read the bottom of it: it names every artifact nothing can reach, every row with no painting, and every sentence quoting a figure the row no longer pays. ⛑ Nothing to regenerate since #290: the tab derives off the running build when you open it |
